...
7. `kubectl apply -f deployment/udaconnect-kafka-broker.yaml` - Set up the service and deployment for kafka broker
8. running `docker build -t kafka-listener kafka_consumer` in modules/kafka-consumer directory and `kubectl apply -f deployment/udaconnect-kafka-consumer.yaml` - Set up the service and deployment for kafka consumer
...
`kubectl get pods` and `kubectl get services` - should both return `udaconnect-app`, `person-api`, `location-api`, `connection-api` and `postgres`
